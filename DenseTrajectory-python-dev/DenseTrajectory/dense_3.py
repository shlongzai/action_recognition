...
##
##		#all_points = numpy.array([[w, h] for w in range(width) for h in range(height)],dtype=numpy.int16)
##		all_points=numpy.stack((numpy.tile(numpy.reshape(numpy.arange(width,dtype=numpy.int16),(width,1)),(1,height)),\
##                                                      numpy.tile(numpy.arange(height,dtype=numpy.int16),(width,1))),axis=2)
		...
		if prev_points.size>0:
			...
			enable_prev_flg = ((prev_points[:,0] < self.x_max[i]) & (prev_points[:,1] < self.y_max[i]))
			#print('prev_points.shape:',prev_points.shape)
			#print('enable_prev_flg.shape:',enable_prev_flg.shape)
			prev_points_1 = (prev_points[enable_prev_flg]/self.DENSE_SAMPLE_PARAM.MIN_DIST).astype(numpy.int16)
			#print('prev_points.shape:',prev_points.shape)
			...
			#enable_point_flg=[True]*all_points.shape[0]
			enable_point_flg=numpy.full((self.width_list[i],self.height_list[i]),True)
			enable_point_flg[prev_points_1[:,0],prev_points_1[:,1]]=numpy.full((prev_points_1.shape[0]),False)
			enable_points = self.all_points[i][enable_point_flg]*self.DENSE_SAMPLE_PARAM.MIN_DIST + self.offset
		else:
                #else:
			# Get feature point candidates
			enable_points = numpy.reshape(self.all_points[i],(-1,2))*self.DENSE_SAMPLE_PARAM.MIN_DIST + self.offset
		#print('enable_points.shape:',enable_points.shape)
...
##		print('gray_frame.shape,eigen_mat.shape:',gray_frame.shape,eigen_mat.shape)
		...
		#print('enable_point_eigen.shape:',enable_point_eigen.shape)

		corner_eigen_flg = (enable_point_eigen > eigen_thresh)
		#print('corner_eigen_flg.shape:',corner_eigen_flg.shape)

		corner_points = enable_points[corner_eigen_flg]
		#print('corner_points.shape:',corner_points.shape)
...
##			for (track, hog, hof, mbhx, mbhy, trj) in zip(track_list.tracks, hog_descs, hof_descs, mbhx_descs, mbhy_descs, trj_descs)]
		for track in track_list.tracks:
			...
			hog=self.hog_create.Extract(hog_integral,  track.points[track.track_num], prev_gray_frame.shape)
			hof=self.hof_create.Extract(hof_integral,  track.points[track.track_num], prev_gray_frame.shape)
			mbhx=self.mbh_create.Extract(mbhx_integral,  track.points[track.track_num], prev_gray_frame.shape)
			mbhy=self.mbh_create.Extract(mbhy_integral,  track.points[track.track_num], prev_gray_frame.shape)
			trj=self.trj_create.Extract(flow_warp,  track.points[track.track_num],image_scale)
			track.ResistDescriptor(hog, hof, mbhx, mbhy, trj)
	...
	def __AddTrackPoints(self, flow, track_list, image_size):
		if track_list.tracks:
			index = numpy.array([track.points[track.track_num,:] for track in track_list.tracks],dtype=numpy.int16)
...
##			index[:,1] = numpy.clip(index[:,1], None, image_size[0] - 1)为啥要分开写
##			index[:,0]=numpy.clip(index[:,0],0,image_size[1] - 1)
##			index[:,1] = numpy.clip(index[:,1], 0, image_size[0] - 1)
			flow_pts = flow[index[:,0], index[:,1]]
			flow_pts = numpy.vstack((flow_pts[:,1], flow_pts[:,0])).transpose()
			track_pts = index + flow_pts
	...
	def __ResistTracks(self, prev_gray_frame, track_list,i):
		...
		[track_list.ResistTrack(dense_pts[idx]) for idx in range(dense_pts.shape[0])]#增加起始点
...
##		self.flow_create.get_para(pry_image_creator.image_sizes,pry_image_creator.image_scales)
		...
		# Dense Sampling
		pyr_dense_pts = [self.__DenseSample(a,self.empty_array,i) for i,a in enumerate(prev_pyr_gray_frame)]
		# Tracking points store
