...
class DenseTrajectory:
	...
	def __DenseSample(self, gray_frame, prev_points=None):
		...
		if prev_points != numpy.array([]):
			...
			enable_prev_flg = ((prev_points[:,0] < x_max) & (prev_points[:,1] < y_max))
			#print('prev_points.shape:',prev_points.shape)
			#print('enable_prev_flg.shape:',enable_prev_flg.shape)
			prev_points = (prev_points[enable_prev_flg]/self.DENSE_SAMPLE_PARAM.MIN_DIST).astype(numpy.int16)
			#print('prev_points.shape:',prev_points.shape)
			...
			enable_point_flg=numpy.full((width,height),True)
			enable_point_flg[prev_points[:,0],prev_points[:,1]]=[False]*prev_points.shape[0]
			enable_points = all_points[enable_point_flg]*self.DENSE_SAMPLE_PARAM.MIN_DIST + offset
		else:
                #else:
			# Get feature point candidates
			enable_points = numpy.reshape(all_points,(-1,2))*self.DENSE_SAMPLE_PARAM.MIN_DIST + offset
