...
class DenseTrajectory:
	...
	def __DenseSample(self, gray_frame, prev_points,i):
		...
		if self.frame_num-self.frame_count<15:
                        print(1)
                        return numpy.array([])
		...
		#print(numpy.sum(numpy.square(all_points-numpy.reshape(self.all_points[i],(-1,2)))),x_max,self.x_max[i],y_max,self.y_max[i])	
...
##			prev_points_3 = (cast_prev_points[enable_prev_flg]/self.DENSE_SAMPLE_PARAM.MIN_DIST).astype(numpy.int64)
##			#print('prev_points.shape:',prev_points.shape)
##			# Get points that do not match previous feature points as candidates
##			prev_point_list = prev_points_3.tolist()
##			enable_point_flg=[True]*all_points.shape[0]
##			for j in prev_point_list:
##				enable_point_flg[j[0]*height+j[1]]=False
##			enable_points = all_points[enable_point_flg]*self.DENSE_SAMPLE_PARAM.MIN_DIST + offset
		...
		#print(numpy.sum(numpy.square(enable_points-enable_points2)))
